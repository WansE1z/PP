...
(define-struct Bird (pos v-y) #:transparent)
(define-struct Pipe (gap-y x) #:transparent)
(define-struct AllPipes (pipes) #:transparent)
(define-struct State (Bird AllPipes Score) #:transparent)
...
(define (get-initial-state)
  (let* ([bird-initial (Bird (make-posn bird-x bird-initial-y)0)]
         [pipe-initial (list (Pipe (+ added-number (random random-threshold)) scene-width))]
         [score-initial 0])
  (State bird-initial
         pipe-initial
         score-initial)))
...
(define (next-state-bird bird gravity)
  (match-let* ([(Bird pos v-y) bird]
               [x (posn-x pos)]
               [y (+ (get-bird-y bird) (Bird-v-y bird))]
               [gravity (+ (Bird-v-y bird) gravity)]
               [posBird (make-posn x y)])
...
(define (next-state-bird-onspace bird momentum)
  (match-let* ([(Bird pos v-y) bird]
         [momentum (- momentum)])
    (struct-copy Bird bird [v-y momentum])
   )
)
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (match-let* ([(State bird pipes score) current-state])             
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (p)
         (match-let* ([pipe-x (+ (get-pipe-x p) (- scroll-speed))])
           (struct-copy Pipe p [x pipe-x])))
       pipes))
...
(define (clean-pipes pipes)
  (filter (lambda (p)
            (match-let* ([pipe-x (+ (get-pipe-x p) pipe-width)])
                          (> pipe-x 0)))
          pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (let* ([x (+ (+ (get-pipe-x(last-pipe pipes)) pipe-gap) pipe-width)]
...
(define (invalid-state? state)
  (or (check-ground-collision (get-bird state))
          (check-pipe-collisions (get-bird state)
                                 (get-pipes state)))
  )
...
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
      #f
      (let* ([posStSusBird (make-posn bird-x (get-bird-y bird))]
             [posDrJosBird (make-posn (+ bird-x bird-width)(+ (get-bird-y bird) bird-height))]
             [posStSusPipeDeSus  (make-posn (get-pipe-x (car pipes))
                                                     0)]
             [posDrJosPipeDeSus (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
                                                     (get-pipe-gap-y (car pipes)))]
             [posStSusPipeDeJos (make-posn (get-pipe-x (car pipes))
                                                     (+ (get-pipe-gap-y (car pipes)) pipe-self-gap))]
             [posDrJosPipeDeJos (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
                                                     ground-y)])
      (if (or (check-collision-rectangles posStSusBird      posDrJosBird
                                          posStSusPipeDeSus posDrJosPipeDeSus)
              (check-collision-rectangles posStSusBird      posDrJosBird
                                          posStSusPipeDeJos posDrJosPipeDeJos)
              )
          #t
...
(define (add-score state)
  (+ (get-score state) 0.1))

(define (next-state state)
  (State (next-state-bird (get-bird state) initial-gravity)
...
(define (draw-frame state)
  (let* ([xBird (+ bird-x  (quotient bird-width 2))]
         [yBird (+ (get-bird-y (get-bird state)) (quotient bird-height 2))]
         [xGround (quotient scene-width 2)]
         [yGround (+ ground-y (quotient ground-height 2))])
  (place-images
   (list bird-image
         ground-image
         (score-to-image (get-score state)))
   (list (make-posn xBird yBird)
         (make-posn xGround yGround)
...
(define (place-pipes pipes scene)
  (if (null? pipes)
      scene
      (let* ([lastEl (cdr pipes)]
             [firstEl (car pipes)]
             [pipeUpImg (rectangle pipe-width (get-pipe-gap-y firstEl) "solid" "green")]
             [pipeDownImg (rectangle pipe-width ( - ( - scene-height (get-pipe-gap-y firstEl))pipe-self-gap)  "solid" "green")]
             [pipe-x (+ (get-pipe-x firstEl) (/ pipe-width 2))]
             [pipeUp-y (quotient (get-pipe-gap-y firstEl) 2)]
             [pipeDown-y (+ (+ (get-pipe-gap-y firstEl) pipe-self-gap)(quotient (- (- scene-height pipe-self-gap) (get-pipe-gap-y firstEl)) 2))]
             [posPipeUp (make-posn pipe-x pipeUp-y)]
             [posPipeDown (make-posn pipe-x pipeDown-y)])
        (place-pipes lastEl (place-images
                            (list pipeUpImg pipeDownImg)
                            (list posPipeUp posPipeDown)
                            scene))
      )
   )
)
