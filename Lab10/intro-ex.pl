...
myReverse([],[]).
myReverse([H|T], RevList):-
    myReverse(T, RevT), myConcat(RevT, [H], RevList).
...
extract([],_,_,[]).
extract([H | _], 0, 0, [H]).
extract([H | T], 0, End, [H | Result]) :- EndNew is End - 1, extract(T, 0, EndNew, Result).
extract([_ | T], Start, End, Result) :- StartNew is Start -1 , EndNew is End - 1, extract(T, StartNew, EndNew, Result).
...
factorial(0,1).
factorial(N, Fact) :- N2 is N - 1, factorial(N2, Fact1), Fact is Fact1 * N.
...
factorial2(0,1).
factorial2(N, Fact) :- factorial2(N2, Fact2), N is N2 + 1, Fact is Fact2 * N.
...
sublista([],[]).
sublista([H | T],[H | SubList]):- sublista(T, SubList).
