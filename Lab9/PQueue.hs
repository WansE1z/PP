...
class (Ord a) => PQueue pq a where
    ...
    toList pQueue = 
      if isEmpty pQueue 
        then [] 
        else (fromJust ( top pQueue)) : (toList ( pop pQueue))
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert elem (LPQ lpq)
      | null lpq = LPQ [elem]
      | fst elem >= fst (head lpq) = LPQ $ elem : lpq
      | otherwise = LPQ $ head lpq : toList (insert elem ( LPQ ( tail lpq)))
    ...
    pop (LPQ lpq) = LPQ(tail lpq)

    isEmpty (LPQ lpq) = null lpq
...
merge tree1@ (Node root1 (par1, value1) left1 right1) tree2@(Node _ (par2, _) _ _) = 
  if par1 <= par2 
    then merge tree2 tree1
    else 
      if rank left1 >= rank mergedTree 
        then (Node (rank mergedTree + 1) (par1, value1) left1 mergedTree)
        else (Node (rank left1 + 1) (par1, value1) mergedTree left1)
    where
      mergedTree = merge right1 tree2

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node root val left right) = inorder left ++ [val] ++ inorder right 
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert el = merge (Node 1 el empty empty)

    top pQueue = 
      if isEmpty pQueue
        then Nothing
        else Just (nodeVal pQueue)

    pop pQueue = merge (left pQueue) (right pQueue)
