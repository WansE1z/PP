...
safe(_, []):-!.
safe(X1/Y1/S1, [X2/Y2/S2 | Others]) :- safe(X1/Y1/S1, Others), (S1 \== S2; (S1 == S2, X1 \== X2, Y1 \== Y2)).
...
% S1.
next_state(misionari, Init, Next):- parseState(Init, M, NMEST1, NCEST1, NMVEST1, NCVEST1),
                                    M == est, boat(NM, NC),
                                    NMEST1 >= NM, NCEST1 >= NC,
                                    DIST1 is NMEST1 - NM, DIST2 is NCEST1 - NC,
                                    DIST3 is NMVEST1 + NM, DIST4 is NCVEST1 + NC,
                                    safeMisionari(DIST1, DIST2),
                                    safeMisionari(DIST3, DIST4),
                                    Next = state(vest, DIST1, DIST2, DIST3, DIST4).

next_state(misionari, Init, Next):- parseState(Init, M, NMEST1, NCEST1, NMVEST1, NCVEST1),
                                    M == vest, boat(NM, NC),
                                    NMVEST1 >= NM, NCVEST1 >= NC,
                                    DIST3 is NMVEST1 - NM, DIST4 is NCVEST1 - NC,
                                    DIST1 is NMEST1 + NM, DIST2 is NCEST1 + NC,
                                    safeMisionari(DIST3, DIST4),
                                    safeMisionari(DIST1, DIST2),
                                    Next = state(est, DIST1, DIST2, DIST3, DIST4).
