...
au_pusca(ListaNume) :- findall(N, inarmat(N,pusca), ListaNume).
...
au_arma(Arma, ListaNume) :- findall(N, inarmat(N, Arma), ListaNume).

au_marca(Marca, ListaNume) :- findall(N, conduce(N, Marca), ListaNume).
...
arme_bmw(ListaArme) :- au_marca(bmw,L),
                       findall(A, (member(N, L), inarmat(N, A)), ListaArme).
...
arme_marca(Marca, ListaArme) :- setof(A, N ^ (conduce(N, Marca), inarmat(N, A)), ListaArme).
...
suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma), arme_marca(Marca, L),
                             forall(member(A, L), au_arma(A, [_, _ | _])).
