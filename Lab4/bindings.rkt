...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((len (get-line-segment))
         (startpct (get-start-point len))
         (endpct (get-end-point len)))
    (sqrt (+ (expt ( - (car startpct) (car endpct)) 2) (expt ( - (cdr startpct) (cdr endpct)) 2))) 
    )
  )
...
(define (num-concat x y)
  (let ((concat (λ (x y ) (string->number( string-append( number->string x)(number->string y))))))
  (concat x y)));; Nu stergeți această linie.
...
(define (3-sequence-max numbers separator)
  (let*-values (((separate) (λ (x) (not (= x separator))))
                ((1seq s1) (splitf-at numbers separate))
                ((2seq s2) (splitf-at (cdr s1) separate))
                ((3seq s3) (splitf-at (cdr s2) separate)))
    (max (max (sumL 1seq) (sumL 2seq)) (sumL 3seq))))
